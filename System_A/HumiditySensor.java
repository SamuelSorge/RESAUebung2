...
class HumiditySensor {
	...
	public static void main(String args[]) {
		...
		MessageManagerInterface em = null; // Interface object to the message
											// manager
		boolean HumidifierState = false; // Humidifier state: false == off, true
											// == on
		boolean DehumidifierState = false; // Dehumidifier state: false == off,
											// true == on
		...
		if (em != null) {
			...
			MessageWindow mw = new MessageWindow("Humidity Sensor", WinPosX,
					WinPosY);
			...
			while (!Done) {
				...
				mw.WriteMessage("Current Relative Humidity:: "
						+ RelativeHumidity + "%");
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Msg.GetMessageId() == -4) {
						if (Msg.GetMessage().equalsIgnoreCase("H1")) // humidifier
																		// on
						{
						...
						if (Msg.GetMessage().equalsIgnoreCase("H0")) // humidifier
																		// off
						{
						...
						if (Msg.GetMessage().equalsIgnoreCase("D1")) // dehumidifier
																		// on
						{
						...
						if (Msg.GetMessage().equalsIgnoreCase("D0")) // dehumidifier
																		// off
						{
				...
				// Now we trend the relative humidity according to the status of
				// the
				...
				// Here we wait for a 2.5 seconds before we start the next
				// sample
	 ...
	 * simulation with random floating point humidity values between 0.1 and
	 * 0.9.
	 *
	 * Arguments: None.
	 *
