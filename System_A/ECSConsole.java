...
public class ECSConsole {
	public static void main(String args[]) {
		...
		float TempRangeHigh = (float) 100.0; // These parameters signify the
												// temperature and humidity
												// ranges in terms
		float TempRangeLow = (float) 0.0; // of high value and low values. The
											// ECSmonitor will attempt to
											// maintain
		float HumiRangeHigh = (float) 100.0; // this temperature and humidity.
												// Temperatures are in degrees
												// Fahrenheit
		float HumiRangeLow = (float) 0.0; // and humidity is in relative
											// humidity percentage.
		...
		if (Monitor.IsRegistered()) {
			...
			while (!Done) {
				...
				System.out.println("\n\n\n\n");
				System.out
						.println("Environmental Control System (ECS) Command Console: \n");
				...
				System.out.println("Set Temperature Range: " + TempRangeLow
						+ "F - " + TempRangeHigh + "F");
				System.out.println("Set Humidity Range: " + HumiRangeLow
						+ "% - " + HumiRangeHigh + "%\n");
					...
					while (Error) {
						...
						while (Error) {
							...
							if (UserInput.IsNumber(Option)) {
								Error = false;
								TempRangeLow = Float.valueOf(Option)
										.floatValue();
						...
						while (Error) {
							...
							if (UserInput.IsNumber(Option)) {
								Error = false;
								TempRangeHigh = Float.valueOf(Option)
										.floatValue();
						...
						if (TempRangeLow >= TempRangeHigh) {
							...
							Monitor.SetTemperatureRange(TempRangeLow,
									TempRangeHigh);
				...
				if (Option.equals("2")) {
					...
					while (Error) {
						...
						while (Error) {
							...
							if (UserInput.IsNumber(Option)) {
								Error = false;
								HumiRangeLow = Float.valueOf(Option)
										.floatValue();
						...
						while (Error) {
							...
							if (UserInput.IsNumber(Option)) {
								Error = false;
								HumiRangeHigh = Float.valueOf(Option)
										.floatValue();
						...
						if (HumiRangeLow >= HumiRangeHigh) {
							...
							Monitor.SetHumidityRange(HumiRangeLow,
									HumiRangeHigh);
