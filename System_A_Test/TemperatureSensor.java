...
class TemperatureSensor {
	public static void main(String args[]) {
		...
		if (em != null) {
			...
			MessageWindow mw = new MessageWindow("Temperature Sensor", WinPosX,
					WinPosY);
			...
			mw.WriteMessage("   Initial Temperature Set:: "
					+ CurrentTemperature);
			...
			while (!Done) {
				...
				mw.WriteMessage("Current Temperature::  " + CurrentTemperature
						+ " F");
				...
				// or chiller has been turned on/off. Note that we get all the
				// messages
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Msg.GetMessageId() == -5) {
						if (Msg.GetMessage().equalsIgnoreCase("H1")) // heater
																		// on
						{
							HeaterState = true;
						...
						if (Msg.GetMessage().equalsIgnoreCase("H0")) // heater
																		// off
						{
							HeaterState = false;
						...
						if (Msg.GetMessage().equalsIgnoreCase("C1")) // chiller
																		// on
						{
							ChillerState = true;
						...
						if (Msg.GetMessage().equalsIgnoreCase("C0")) // chiller
																		// off
						{
							ChillerState = false;
	 ...
	 * simulation with random floating point temperature values between 0.1 and
	 * 0.9.
	 *
	 * Arguments: None.
	 *
	...
	static private void PostTemperature(MessageManagerInterface ei,
			float temperature) {
