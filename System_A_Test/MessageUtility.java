...
public class MessageUtility {
	public static void main(String args[]) {
		...
		MessageManagerInterface ef = null;// Interface object to the message
											// manager
		...
		if (MsgMgrIP.length() == 0) {
			try {
				// Here we create a message manager interface object. This
				// assumes
		...
		if (ef != null) {
			...
			while (!Done) {
				...
				// Here we print out the participants ID number. This is
				// established when
				// the message interface is instantiated. If the participant is
				// not
				...
				// Here we print out the participants registrations time. This
				// is actual time
				// that the message interface was instantiated. If the
				// participant is not
				// registered an exception is thrown.
				...
				if (Option.equals("3")) {
					// Here we get an message ID from the user,... it has to be
					// an integer,
					// so the input is verified. If the input is not an integer,
					// we chastise
					...
					while (Error) {
						...
						if (UserInput.IsNumber(Option)) {
							...
							System.out
									.println("Please enter an integer value... try again...");

						} // if

					} // while

					// Here ask the user to provide a string message to post
					// with the
					// if message.
				...
				if (Option.equals("4")) {
					// Here we get the message queue for this message interface
					...
					try {
						...
						for (int i = 0; i < qlen; i++) {
							Msg = eq.GetMessage();
							System.out.print((i + 1) + "::Sender ID: "
									+ Msg.GetSenderId());
				...
				if (Option.equalsIgnoreCase("X")) {
					// Here the user is done, so we set the Done flag and
					// unregister
					// the message interface from the message manager. If you
					// fail to
					// unregister, the message manager doesn't know to remove
					// queues.
					// These become dead queues and they collect messages and
					// will messageually
