...
class ECSMonitor extends Thread {
	...
	private float TempRangeHigh = 100; // These parameters signify the
										...
										// terms
	private float TempRangeLow = 0; // of high value and low values. The
									// ECSmonitor will attempt to maintain
	private float HumiRangeHigh = 100; // this temperature and humidity.
										// Temperatures are in degrees
										// Fahrenheit
	private float HumiRangeLow = 0; // and humidity is in relative humidity
									// percentage.
	...
	public void run() {
		...
		float CurrentTemperature = 0; // Current temperature as reported by the
										// temperature sensor
		float CurrentHumidity = 0; // Current relative humidity as reported by
									// the humidity sensor
		...
		if (em != null) {
			...
			hi = new Indicator("HUMI UNK", mw.GetX() + mw.Width(),
					(int) (mw.Height() / 2), 2);
			...
			while (!Done) {
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Msg.GetMessageId() == 1) // Temperature reading
					{
						try {
							CurrentTemperature = Float
									.valueOf(Msg.GetMessage()).floatValue();
					...
					if (Msg.GetMessageId() == 2) // Humidity reading
					{
						try {
							...
							CurrentHumidity = Float.valueOf(Msg.GetMessage())
									.floatValue();
				...
				mw.WriteMessage("Temperature:: " + CurrentTemperature
						+ "F  Humidity:: " + CurrentHumidity);

				// Check temperature and effect control as necessary

				if (CurrentTemperature < TempRangeLow) // temperature is below
														// threshhold
				{
					...
					if (CurrentTemperature > TempRangeHigh) // temperature is
															// above threshhold
					{
						...
						ti.SetLampColorAndMessage("TEMP OK", 1); // temperature
																	// is within
																	// threshhold
						Heater(OFF);
						Chiller(OFF);
				...
				if (CurrentHumidity < HumiRangeLow) {
					hi.SetLampColorAndMessage("HUMI LOW", 3); // humidity is
																// below
																// threshhold
					Humidifier(ON);
					Dehumidifier(OFF);
					...
					if (CurrentHumidity > HumiRangeHigh) // humidity is above
															// threshhold
					{
						...
						hi.SetLampColorAndMessage("HUMI OK", 1); // humidity is
																	// within
																	// threshhold
						Humidifier(OFF);
						Dehumidifier(OFF);
	...
	/***************************************************************************
	 * CONCRETE METHOD:: SetTemperatureRange Purpose: This method sets the
	 * temperature range
	 *
	 * Arguments: float lowtemp - low temperature range float hightemp - high
	 * temperature range
	 *
	 * Returns: none
	 *
	...
	public void SetTemperatureRange(float lowtemp, float hightemp) {
		...
		mw.WriteMessage("***Temperature range changed to::" + TempRangeLow
				+ "F - " + TempRangeHigh + "F***");

	} // SetTemperatureRange

	/***************************************************************************
	 * CONCRETE METHOD:: SetHumidityRange Purpose: This method sets the humidity
	 * range
	 *
	 * Arguments: float lowhimi - low humidity range float highhumi - high
	 * humidity range
	 *
	 * Returns: none
	 *
	...
	public void SetHumidityRange(float lowhumi, float highhumi) {
		...
		mw.WriteMessage("***Humidity range changed to::" + HumiRangeLow
				+ "% - " + HumiRangeHigh + "%***");
