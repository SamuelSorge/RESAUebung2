...
class HumidityController {
	public static void main(String args[]) {
		...
		MessageManagerInterface em = null; // Interface object to the message
											// manager
		boolean HumidifierState = false; // Heater state: false == off, true ==
											// on
		...
		if (em != null) {
			...
			MessageWindow mw = new MessageWindow(
					"Humidity Controller Status Console", WinPosX, WinPosY);

			// Now we put the indicators directly under the humitity status and
			// control panel

			Indicator hi = new Indicator("Humid OFF", mw.GetX(), mw.GetY()
					+ mw.Height());
			Indicator di = new Indicator("DeHumid OFF", mw.GetX()
					+ (hi.Width() * 2), mw.GetY() + mw.Height());
			...
			while (!Done) {
				...
				// We are looking for MessageIDs = 4, this is a request to turn
				// the
				// humidifier or dehumidifier on/off. Note that we get all the
				// messages
